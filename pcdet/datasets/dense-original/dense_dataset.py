...
class DenseDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, search_str: str ='',
        ...
        self.lidar_folder = f'training/lidar_{self.sensor_type}_{self.signal_type}'
    ...
    def get_image_shape(self, idx):
        ...
        print(str(self.root_split_path / 'training/cam_stereo_left_lut' / ('%s.png' % idx)))

        img_file = self.root_split_path / 'training/cam_stereo_left_lut' / ('%s.png' % idx)
        assert img_file.exists(), f'{img_file} not found'
        return np.array(io.imread(img_file).shape[:2], dtype=np.int32)

    def get_label(self, idx):
        label_file = self.root_split_path / 'training/gt_labels' / 'cam_left_labels_TMP' / ('%s.txt' % idx)
        assert label_file.exists(), f'{label_file} not found'
        return object3d_kitti.get_objects_from_label(label_file, dense=True)

    def get_road_plane(self, idx):
        plane_file = self.root_split_path / 'training/velodyne_planes' / ('%s.txt' % idx)
